...
public class SwipeDismissListView extends ListView {
	...
	private void handleActionUp(MotionEvent ev) {
		...
		final float deltaX = ev.getX() - mDownX;
			...
			ViewPropertyAnimator.animate(mDownView)
					...
					.setListener(new AnimatorListenerAdapter() {
						...
						public void onAnimationEnd(Animator animation) {
							//Item滑出界面之后执行删除
							performDismiss(mDownView, mDownPosition,isLeft);
						}
					});
		} else {
			//将item滑动至开始位置
	 ...
	 * 如果没有做出滑动操作，则修改内容
	 */
	private void performToEdit(int dismissPosition) {
		if (onDismissCallback != null) {
			onDismissCallback.onEdit(dismissPosition);
		}
	}
	 ...
	 * @param isLeft 
	 */
	private void performDismiss(final View dismissView, final int dismissPosition,final boolean isLeft) {
